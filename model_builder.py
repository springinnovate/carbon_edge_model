...
def generate_sample_points_probably_forest_for_carbon_model(
    ...
    while points_remaining > 0:
        ...
        for window_index, point_list in window_index_to_point_list_map.items():
            if not point_list:
                LOGGER.debug(f'not point list on index {window_index}')
                continue
            ...
            for lng, lat in point_list:
                ...
                for index, (raster_path, band, nodata, nodata_replace,
                    ...
                    if index == 0:
                        ...
                        if val < bm_thresh_min or val == nodata:
                            # too small, skip
                        ...
                        if prob < numpy.random.random():
                            ...
                            valid_working_list = False
                            # LOGGER.debug(f'baccini sample not big enough {prob}')
                            break
                    ...
                    if nodata is None or not numpy.isclose(val, nodata):
                        ...
                        valid_working_list = False
                        break
                    # LOGGER.debug(f'*** good data on {raster_path} at {lng} {lat} {x} {y}')
                ...
                if valid_working_list:
                    ...
                    LOGGER.debug(f'******* DID IT ON {lng} {lat}')
...
if __name__ == '__main__':
    ...
    raster_path_nodata_replacement_list = carbon_model_data.create_aligned_base_data(
        baccini_10s_2014_biomass_path, carbon_model_data.ALIGNED_DATA_DIR,
        carbon_model_data.TARGET_BB)
