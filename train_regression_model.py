...
def main():
    ...
    for name, reg in [
            ...
            ('LassoLarsCV', make_pipeline(poly_features, StandardScaler(),  PCA(), LassoLarsCV(max_iter=max_iter, cv=10, eps=1e-3, normalize=False), func=numpy.log, inverse_func=numpy.exp)),
            ('LassoLars', make_pipeline(poly_features, StandardScaler(), PCA(),  LassoLars(alpha=.1, normalize=False, max_iter=max_iter, eps=1e-3), func=numpy.log, inverse_func=numpy.exp)),
            ]:
