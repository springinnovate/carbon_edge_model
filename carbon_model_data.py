...
def fetch_data(target_data_dir, task_graph):
    ...
    Returns:
        List of (file_path, nodata) tuples.
    ...
    for file_id, nodata in \
        ...
        target_file_path = os.path.join(
            target_data_dir, os.path.basename(file_url))
        LOGGER.info(
            f'download_gs for {file_url}, exists? '
            f'{os.path.exists(target_file_path)}')
        download_task = task_graph.add_task(
            func=ecoshard.download_url,
            args=(file_url, target_file_path),
            target_path_list=[target_file_path],
            task_name=f'download {target_file_path}')
        if file_url not in [BACCINI_10s_2014_BIOMASS_URL, ESA_LULC_URL]:
            downloaded_file_list.append((target_file_path, nodata))
